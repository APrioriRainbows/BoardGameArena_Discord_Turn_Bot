...
class BgaCommands(commands.Cog):
    ...
    async def info(self, interaction: discord.Interaction) -> None:
        lines = [
            "## 🎲 BGA Turn Bot",
            "Watches your Board Game Arena tables and @mentions players when it's their turn. Posts final scores when the game ends.",
            "",
            "**Quick start:**",
            "1. Make your BGA table public (spectator mode on)",
            "2. Run `/bga configure` from the channel or thread where you want notifications",
            "3. Paste the full BGA table URL, pick a channel, choose notification types, click **Start watching**",
            "4. Run `/bga link-all` to match each BGA player to their Discord account",
            "",
            "**Setup:**",
            "`/bga configure` - Add a new game or edit settings for an existing one. Opens a form to paste the table URL, pick a notification channel (defaults to current), and choose which notification types to enable.",
            "",
            "**Notification types** (all on by default, toggle in `/bga configure`):",
            "⏳ **Turn notifications** - pings you when it's your turn",
            "✅ **Turn recap** - shows who just completed their turn",
            "🎉 **Final results** - posts scores when the game ends",
            "",
            "**Managing games:**",
            "`/bga unwatch <table_id>` - Stop watching a table",
            "`/bga unwatch-all` - Stop watching all tables in this server (requires Manage Server)",
            "`/bga watchlist` - List all tables currently being watched",
            "`/bga turn` - Manually repost the current turn status",
            "`/bga status` - Show the last known turn state for all watched tables (only you see this)",
            "",
            "**Linking players:**",
            "Linking maps a BGA username to a Discord account so the bot can @mention the right person.",
            "`/bga link-all` - Bulk-link all BGA players from watched games. Shows a searchable Discord member dropdown for each unlinked player, up to 4 per page.",
            "`/bga link-member <discord_user> <bga_name>` - Manually link one player",
            "`/bga unlink-member <discord_user>` - Remove a link",
            "`/bga linked` - List all current Discord/BGA links",
            "",
            "**Debugging:**",
            "`/bga logs` - Show the last 30 raw BGA websocket events. Useful if turn detection seems off or scores are missing after a game ends. Requires `LOG_LEVEL=DEBUG` in the bot's config.",
        ]
